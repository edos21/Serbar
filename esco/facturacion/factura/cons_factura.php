<?php
//factura tomada del modelo de serbar
...
<html lang="es">
	...
	<body>
		...
		if ($reg=mysql_fetch_array($registro)) {
			$regisservicio=mysql_query("select * from escoservicios where n_presupuesto='$reg[n_presupuesto]'",$conexion) or
			die("error en select: ".mysql_error());
			$regispresupuesto=mysql_query("select * from escopresupuesto where n_presupuesto='$reg[n_presupuesto]'",$conexion) or
			die("error en select: ".mysql_error());
		?>
		<div>
			<caption>FACTURA <?php echo $reg['n_factura']?></caption>
			<caption><?php echo $reg['filial']?></caption>
