...
<html lang="es">
	<head>
		<meta charset="UTF-8" />
		<title>Consulta factura - Facturas JJCORDERO</title>
		<link rel="stylesheet" href="../css/administracion.css" type="text/css"/>
	</head>
	<body>
		<?php
		if ($reg=mysql_fetch_array($registro)) {
			$regisservicio=mysql_query("select * from jjservicios where n_presupuesto='$reg[n_presupuesto]'",$conexion) or
			die("error en select: ".mysql_error());
			$regispresupuesto=mysql_query("select * from jjpresupuesto where n_presupuesto='$reg[n_presupuesto]'",$conexion) or
			die("error en select: ".mysql_error());
		?>
		<div>
