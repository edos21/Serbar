...
<html lang="es">
	...
	<body>
		...
		if ($reg=mysql_fetch_array($registro)){
			mysql_query("delete from jjpresupuesto where n_presupuesto='$_REQUEST[presupuesto]'",$conexion) or
			die("error en select: ".mysql_error());
			$registroservicio=mysql_query("select * from jjservicios where n_presupuesto='$_REQUEST[presupuesto]'",$conexion) or
			die ("error en select: ".mysql_error());
			if ($reg2=mysql_fetch_array($registroservicio)){
				mysql_query("delete from jjservicios where n_presupuesto='$_REQUEST[presupuesto]'",$conexion) or
				die("error en select: ".mysql_error());
			}
